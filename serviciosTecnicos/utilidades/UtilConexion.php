...
class UtilConexion {
    ...
    public static $categoriaTipoOrden;
    public static $categoriaTipoMaquina;
    ...
    public function __construct() {
        ...
        self::$categoriaTipoOrden = ['0' => 'Ninguna', '1' => 'Mantenimiento', '2' => 'Impresion', '3' => 'Descanso'];
        self::$categoriaTipoMaquina = ['0' => 'Seleccione un tipo de máquina', '1' => 'Rotativa', '2' => 'Plana'];
    ...
    public function getCategoria() {
        echo json_encode(self::$categoriaTipoOrden);
    }

    public function getTipoMaquina() {
        echo json_encode(self::$categoriaTipoMaquina);
    }

    public function restore() {

        $backup = Utilidades::subirArchivo();  //////////////
        $mensaje = '';
        $usuario = USUARIO;
        $password = CONTRASENA;
        ...
        $eliminarBd = '';
        $this->pdo = new PDO("pgsql:host=" . SERVIDOR . " port=" . PUERTO . " dbname=" . BASE_DATOS, USUARIO, CONTRASENA);
        $fila = $this->pdo->query("SELECT count(*) as existe FROM pg_catalog.pg_database WHERE datname = '" . BASE_DATOS . "'")->fetch();

        if (PHP_OS === "Linux") {
            if ($fila['existe']) {
                // cerrar las sesiones abiertas de dicha bd y eliminarla
                $this->conexion->exec("SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '$bd'");
                $eliminarBd = "dropdb -h localhost -U $usuario $bd";
            }
            // construir el restore
            $backup = PATH_TMP . $backup;
            echo "<br>Archivo que se intenta leer: $backup<br>";
            $script = "
                        export PGPASSWORD=$password
                        $eliminarBd
                        createdb -h localhost -U $usuario $bd
                        pg_restore -v -F c -d $bd -h localhost -U $usuario < \"$backup\"
                    ";
            // ejecutar el restore
            exec($script, $out, $status);
            $mensaje = "<br>Estado: $status<br>" . print_r($out);
        } elseif (PHP_OS === "WINNT") {
            if ($fila['existe']) {
                // cerrar las sesiones abiertas de dicha bd y eliminarla
                $this->pdo->exec("SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '" . BASE_DATOS . "'");
                $eliminarBd = '"' . $this->ruta_pgslq . 'dropdb.exe" -h localhost -U ' . $usuario . " $bd";
            }
            // referenciar la carpeta de los ejecutables de Postgres
            $sql = "SELECT setting FROM pg_settings WHERE name = 'data_directory'";
            if (($rs = $this->pdo->query($sql))) {
                $fila = $rs->fetch(PDO::FETCH_ASSOC);
                $this->ruta_pgslq = "C:/Program Files/PostgreSQL/9.5/bin/";
            }

            $this->tmpWin = DOCUMENT_ROOT . 'tmp/';

            // construir las rutas para los comandos
            $batchRestore = $this->tmpWin . 'restore.bat';
            $createdb = $this->ruta_pgslq . 'createdb.exe';
            $pg_restore = $this->ruta_pgslq . 'pg_restore.exe';
            $backup = $this->tmpWin . $backup;
            // construir el restore como una plantilla
            $script = '
                        @echo off
                        set PGPASSWORD=_password
                        _eliminarBd
                        "_createdb" -U _usuario _bd
                        "_pg_restore" -h localhost -p 5432 -U _usuario -d _bd "_backup"
                        exit
                    ';
            // reemplazar en el script las variables por datos
            $script = str_replace(["_password", "_createdb", "_eliminarBd", "_usuario", "_bd", "_pg_restore", "_backup"], [CONTRASENA, $createdb, $eliminarBd, USUARIO, $bd, $pg_restore, $backup], $script);
// ejecutar el restore
            if (file_put_contents($batchRestore, $script) !== FALSE) {
                // ejecutar el script. Falta: Execute Cmd.exe Silently
                $script = $_SERVER['COMSPEC'] . ' /c START ' . $batchRestore;
                exec($script);
                //unlink($batchRestore);
            }
        }
        // se abre la base de datos actual


        $this->pdo = new PDO("pgsql:host = " . SERVIDOR . " port = " . PUERTO . " dbname = " . BASE_DATOS, USUARIO, CONTRASENA);

        $mensaje .="<br>Ejecución de restore finalizada.";
