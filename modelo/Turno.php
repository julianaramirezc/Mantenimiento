...
class Turno {
    ...
    function add($param) {
        ...
        $sql = "INSERT INTO turno(hora_inicio, hora_fin) VALUES ('$hora_inicio', '$hora_fin')";
    ...
    function edit($param) {
        ...
        extract($param);
        $sql = "UPDATE turno SET  hora_inicio='$hora_inicio', hora_fin='$hora_fin' WHERE turno.id_turno = $id";  // <-- el ID de la fila asignado en el SELECT permite construir la condición de búsqueda del registro a modificar
        $conexion->getPDO()->exec($sql);
        echo $conexion->getEstado();
    }
    ...
    function del($param) {
        ...
        extract($param);
        $conexion->getPDO()->exec("DELETE FROM turno WHERE id_turno = $id");
        echo $conexion->getEstado();
    }
    ...
    public function getSelect($param) {
        ...
        foreach ($conexion->getPDO()->query("SELECT id_turno, hora_inicio || ' - ' || hora_fin AS turno FROM turno ORDER BY hora_inicio") as $fila) {
